...
pub fn sys_mutex_lock(mutex_id: usize) -> isize {
    ...
    // println!("len: {}", process_inner.mutex_need.len());
    process_inner.mutex_need[tid][mutex_id] += 1;
    if process_inner.mutex_deadlock() {
        process_inner.mutex_need[tid][mutex_id] -= 1;
        drop(process_inner);
        drop(process);
        return -0xDEAD;
...
pub fn sys_semaphore_create(res_count: usize) -> isize {
        ...
        .map(|(id, _)| id)
    {
        ...
        process_inner.semaphore_work[id] = res_count;
        id
    } else {
        process_inner
            .semaphore_list
        ...
        process_inner.semaphore_work.push(res_count);
...
pub fn sys_semaphore_up(sem_id: usize) -> isize {
    ...
    let tid = current_task().unwrap().inner_exclusive_access().res.as_ref().unwrap().tid;
    process_inner.semaphore_allocated[tid][sem_id] -= 1;
...
pub fn sys_semaphore_down(sem_id: usize) -> isize {
    ...
    if process_inner.semaphore_deadlock() {
        process_inner.semaphore_need[tid][sem_id] -= 1;
        drop(process_inner);
        drop(process);
        return -0xDEAD;
